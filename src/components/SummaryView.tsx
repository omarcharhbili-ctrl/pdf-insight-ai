    ...
    >
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-primary/8 text-primary text-[11px] font-medium">
            <Sparkles className="w-3 h-3" />
            AI-Generated Summary
          </div>
          <h2 className="text-xl font-semibold text-foreground tracking-tight">{summary.title}</h2>
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5"><Clock className="w-3 h-3" />{summary.readTime}</span>
            <span className="flex items-center gap-1.5"><BookOpen className="w-3 h-3" />{summary.pages} pages</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {summary.tags.map((tag) => (
            <span key={tag} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-secondary border border-border text-[11px] font-medium text-secondary-foreground">
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="space-y-2.5">
          <h3 className="text-[11px] font-semibold text-muted-foreground uppercase tracking-[0.1em]">Key Points</h3>
          <div className="space-y-1.5">
            {summary.keyPoints.map((point, i) => (
              <motion.div
                ...
                className="flex gap-3 p-3 rounded-lg bg-secondary/40 border border-border"
              >
                <span className="w-5 h-5 rounded bg-primary/8 text-primary text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5 mono">
                  {i + 1}
                </span>
                <p className="text-[13px] text-secondary-foreground leading-relaxed">{point}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        {/* Full Summary */}
        <div className="space-y-2.5">
          <h3 className="text-[11px] font-semibold text-muted-foreground uppercase tracking-[0.1em]">Overview</h3>
